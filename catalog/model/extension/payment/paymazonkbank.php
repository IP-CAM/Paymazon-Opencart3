...
class ModelExtensionPaymentPaymazonkbank extends Model {
    public function getMethod($address, $total) {
        ...
        if ($this->config->get('payment_paymazonkbank_total') > 0 && $this->config->get('payment_paymazonkbank_total') > $total) {
            $status = false;
	...
	public function insertNewPaymazonTransaction($order_id, $request_id, $input_params = array()) {
		...
		$sql = sprintf("INSERT INTO %s(order_id, request_id, paymazon_payment_code, paymazon_payment_id, paymazon_payment_status, paymazon_datetime_insert, paymazon_datetime_update) VALUES('%s', '%s', '%s', '%s', '%s', NOW(), NOW())",
